...
__global__ void reduceAtomic(int *g_idata, int *g_result, unsigned int n)
{
    ...
    if (tid == 0) {
        atomicAdd(g_result, smem[0]);   // 关键：所有block原子累加到同一个地址
    }
}
...
int main(int argc,char** argv)
{
    ...
    reduceAtomic<<<grid.x, block>>>(idata_dev, d_result, size);
    cudaDeviceSynchronize();

    int atomic_sum;
    cudaMemcpy(&atomic_sum, d_result, sizeof(int), cudaMemcpyDeviceToHost);
    printf("sum = %d\n", atomic_sum);
